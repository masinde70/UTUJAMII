...
export default async function NewsPage() {
  let articles = [];
  try {
    articles = await client.fetch(NEWS_QUERY);
  } catch (error) {
    console.warn("Sanity fetch failed (likely missing project ID). Returning empty feed.");
  }
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        {articles.length === 0 ? (
          <div className="text-center py-12 bg-surface border border-muted rounded-2xl">
            <p className="text-muted-foreground">No news articles found. Create some in the Studio!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map((article: any) => (
              <Link href={`/news/${article.slug.current}`} key={article._id} className="group flex flex-col bg-surface border border-muted rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
