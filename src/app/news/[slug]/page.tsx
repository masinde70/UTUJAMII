...
import { NEWS_BY_SLUG_QUERY } from "@/sanity/queries";
...
// Custom styling for PortableText to create a premium editorial feel
const ptComponents = {
  block: {
...
export default async function NewsArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let article = null;
  try {
    article = await client.fetch(NEWS_BY_SLUG_QUERY, { slug: resolvedParams.slug });
  } catch (error) {
    ...
    <article className="pb-24 bg-background w-full overflow-hidden">
      ...
      <div className="relative w-full h-[60vh] min-h-[500px] flex flex-col justify-end mb-16 px-4 md:px-12 pb-16 lg:pb-24">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col items-start pt-32">
          <Link href="/news" className="inline-flex items-center text-white/70 hover:text-white transition-colors mb-8 font-medium tracking-wide uppercase text-xs">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-12">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          {/* Social Share / Meta Sidebar (Sticky) */}
          <div className="hidden lg:block lg:w-1/4">
            <div className="sticky top-32 flex flex-col gap-8 bg-surface border border-border p-8 rounded-3xl shadow-sm">
              <div className="pb-8 border-b border-border">
                <p className="text-xs font-bold tracking-widest uppercase text-muted-foreground mb-3">Published</p>
                <p className="font-serif text-2xl text-foreground font-semibold">{publishDate}</p>
              </div>
              <div>
                <p className="text-xs font-bold tracking-widest uppercase text-muted-foreground mb-4">Share Article</p>
                <div className="flex gap-4">
                  <button className="w-12 h-12 rounded-full bg-background border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <Twitter size={18} />
                  </button>
                  <button className="w-12 h-12 rounded-full bg-background border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <Linkedin size={18} />
          ...
          <div className="lg:w-3/4">
            <div className="max-w-3xl pt-4">
              {article.body ? (
                ...
                <p className="text-xl text-muted-foreground italic">No detailed content provided for this article.</p>
              )}
            </div>
          </div>
        </div>
        ...
        {article.galleryUrls && article.galleryUrls.length > 0 && (
          <div className="max-w-7xl mx-auto mt-32 pt-24 border-t border-border">
            <div className="text-center mb-16">
              <span className="text-accent font-semibold tracking-wider uppercase text-sm mb-4 block">
                Visual Impact
              </span>
              <h3 className="font-serif text-4xl md:text-5xl font-bold mb-4 text-foreground">Event Gallery</h3>
              <p className="text-muted-foreground text-xl font-light">Highlights and moments captured directly from the field.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 auto-rows-[300px]">
              {article.galleryUrls.map((url: string, index: number) => {
                // Determine span classes to create a beautiful masonry-like staggered block layout
                ...
                return (
                  <div key={index} className={`bg-muted rounded-3xl overflow-hidden relative group shadow-md hover:shadow-2xl transition-all duration-700 ${spanClass}`}>
                    ...
                    />
                    {/* Premium vignette effect on hover */}
