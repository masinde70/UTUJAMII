        ...
        {articles.length === 0 ? (
          ...
          <div className="space-y-20 md:space-y-28">
            {articles.map((article, i) => (
              <DispatchEntry key={article.id} article={article} index={i} />
            ))}
          </div>
        )}
      </div>
...
function DispatchEntry({ article, index }: { article: News; index: number }) {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-12 gap-x-6 md:gap-x-12 gap-y-8 items-center">
        ...
        {/* Text */}
        <div
          className={`col-span-12 md:col-span-5 flex flex-col ${reverse ? "md:order-1" : ""}`}
        >
          <div className="flex items-center gap-3 mb-5">
            <span className="text-[10px] md:text-[11px] uppercase tracking-[0.28em] text-foreground/50 tabular-nums">
